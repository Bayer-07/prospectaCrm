...
export function EmailPage() {
  ...
  return <div className="email-page"><div className="channel-disabled"><span><AlertCircle size={19} /></span><div><strong>Canal de e-mail preparado, mas desativado</strong><p>Você pode criar modelos e rascunhos. Agendamento e envio serão liberados após configurar um provedor SMTP/API.</p></div><Button variant="secondary" disabled><Settings2 size={15} />Configurar provedor</Button></div><div className="toolbar"><div className="segmented" role="group" aria-label="Alternar visão de e-mail"><button className={view === 'templates' ? 'active' : ''} aria-pressed={view === 'templates'} onClick={() => setView('templates')}>Modelos</button><button className={view === 'drafts' ? 'active' : ''} aria-pressed={view === 'drafts'} onClick={() => setView('drafts')}>Campanhas em rascunho</button></div>{view === 'templates' && <Button onClick={() => setModal(true)}><Plus size={15} />Novo modelo</Button>}</div>{view === 'templates' ? templates.length ? <div className="template-grid">{templates.map((template) => <article key={template.id}><span><Mail size={18} /></span><h3>{template.name}</h3><strong>{template.subject}</strong><p>{template.html.replace(/<[^>]+>/g, '').slice(0, 120)}</p><footer><small>Atualizado {dateTime(template.updatedAt)}</small><Button variant="secondary" disabled><Send size={14} />Enviar</Button></footer></article>)}</div> : <Empty icon={<FileText />} title="Nenhum modelo de e-mail" description="Prepare assuntos e conteúdos enquanto o provedor de envio é definido." action={<Button onClick={() => setModal(true)}>Criar modelo</Button>} /> : <Empty icon={<Mail />} title="Nenhuma campanha em rascunho" description="As campanhas poderão ser preparadas aqui quando o provedor de e-mail for configurado." />}{modal && <TemplateModal onClose={() => setModal(false)} onCreated={() => { setModal(false); client.invalidateQueries({ queryKey: ['email-templates'] }); }} />}</div>;
}
