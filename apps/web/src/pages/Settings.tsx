...
import { AlertTriangle, Check, Copy, KeyRound, Link2, MessageSquareText, MoreHorizontal, Plus, QrCode, RefreshCw, ShieldCheck, Smartphone, Trash2, UserPlus, Users } from 'lucide-react';
...
export function SettingsPage() {
  ...
  const tab = requestedTab && ['users', 'roles', 'whatsapp', 'api', 'security'].includes(requestedTab) ? requestedTab : 'users';
  const selectTab = (next: string) => setSearchParams({ tab: next }, { replace: true });
  return <div className="settings-layout"><aside className="settings-nav"><button className={tab === 'users' ? 'active' : ''} onClick={() => selectTab('users')}><Users size={16} />Usuários e equipes</button><button className={tab === 'roles' ? 'active' : ''} onClick={() => selectTab('roles')}><ShieldCheck size={16} />Papéis e permissões</button><button className={tab === 'whatsapp' ? 'active' : ''} onClick={() => selectTab('whatsapp')}><MessageSquareText size={16} />WhatsApp</button><button className={tab === 'api' ? 'active' : ''} onClick={() => selectTab('api')}><KeyRound size={16} />API e webhooks</button><button className={tab === 'security' ? 'active' : ''} onClick={() => selectTab('security')}><ShieldCheck size={16} />Segurança</button></aside><section className="settings-content">{tab === 'users' && <UsersSettings />}{tab === 'roles' && <RolesSettings />}{tab === 'whatsapp' && <WhatsappSettings />}{tab === 'api' && <ApiSettings />}{tab === 'security' && <SecuritySettings />}</section></div>;
}
