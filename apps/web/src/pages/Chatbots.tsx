...
import {
  ...
  Pause, Play, Plus, Save, Send, Sparkles, Tag, Trash2, UserRoundCheck,
} from 'lucide-react';
...
export function ChatbotsPage() {
  ...
  return <div className="automations-page chatbot-page"><div className="toolbar"><div className="segmented"><button className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>Todos</button><button className={filter === 'PUBLISHED' ? 'active' : ''} onClick={() => setFilter('PUBLISHED')}>Ativos</button><button className={filter === 'DRAFT' ? 'active' : ''} onClick={() => setFilter('DRAFT')}>Rascunhos</button><button className={filter === 'PAUSED' ? 'active' : ''} onClick={() => setFilter('PAUSED')}>Pausados</button></div><Button onClick={() => setModal(true)}><Plus size={15} />Novo chatbot</Button></div>
    <div className="ai-ready-note"><Sparkles size={17} /><div><strong>Motor por regras</strong><p>O fluxo usa regras determinísticas. A camada de provedor já está separada para receber um motor de IA futuramente.</p></div></div>
    {chatbots.length ? <div className="workflow-grid chatbot-grid">{chatbots.map((chatbot) => <button key={chatbot.id} onClick={() => setSelectedId(chatbot.id)}><div className="workflow-icon"><Bot size={20} /></div><div className="workflow-card-header"><Status value={chatbot.status} /><span>v{chatbot.versions[0]?.version || 1}</span></div><h3>{chatbot.name}</h3><p>{chatbot.description || 'Atendimento automático por regras'}</p><footer><span>{chatbot.instance.name} · {chatbot._count?.sessions || 0} atendimentos</span><span>Atualizado {dateTime(chatbot.updatedAt)}</span></footer></button>)}</div> : <Empty icon={<Bot />} title={all.length ? 'Nenhum chatbot neste filtro' : 'Crie seu primeiro chatbot'} description="Monte o atendimento em um mapa visual e transfira para a equipe quando necessário." action={<Button onClick={() => setModal(true)}>Novo chatbot</Button>} />}
    {modal && <CreateChatbotModal metadata={metadata.data!.data} onClose={() => setModal(false)} onCreated={(chatbot) => { setModal(false); setSelectedId(chatbot.id); }} />}</div>;
}

function CreateChatbotModal({ metadata, onClose, onCreated }: { metadata: Metadata; onClose(): void; onCreated(chatbot: ChatbotRecord): void }) {
...
function ChatbotBuilder({ chatbotId, metadata, onBack }: { chatbotId: string; metadata: Metadata; onBack(): void }) {
  ...
  const save = useMutation({ mutationFn: () => api(`/chatbots/${chatbotId}/draft`, { method: 'PATCH', body: JSON.stringify({ graph }) }), onSuccess: () => query.refetch() });
  const publish = useMutation({ mutationFn: async () => { await api(`/chatbots/${chatbotId}/draft`, { method: 'PATCH', body: JSON.stringify({ graph }) }); return api(`/chatbots/${chatbotId}/publish`, { method: 'POST' }); }, onSuccess: () => query.refetch() });
  const changeStatus = useMutation({ mutationFn: (status: string) => api(`/chatbots/${chatbotId}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }), onSuccess: () => query.refetch() });
  ...
  const addNode = (type: string) => { const definition = nodeDefinitions.find((item) => item.type === type)!; const id = `${type}-${Date.now()}`; const data: FlowData = { label: definition.label, subtitle: definition.subtitle }; if (type === 'message') data.text = 'Olá, {{nome}}! Como posso ajudar?'; if (type === 'question') data.text = 'Por favor, conte brevemente o que você precisa.'; if (type === 'condition') { data.operator = 'contains'; data.value = 'vendas'; } setNodes((current) => [...current, { id, type, position: { x: 300 + current.length * 45, y: 90 + (current.length % 5) * 110 }, data }]); setSelectedNodeId(id); };
  ...
  const deleteSelected = () => { if (!selectedNode || selectedNode.type === 'trigger') return; setNodes((current) => current.filter((node) => node.id !== selectedNode.id)); setEdges((current) => current.filter((edge) => edge.source !== selectedNode.id && edge.target !== selectedNode.id)); setSelectedNodeId(null); };
  if (query.isLoading) return <PageLoading />;
  const chatbot = query.data!.data; const error = save.error || publish.error || changeStatus.error;
  return <div className="workflow-builder chatbot-builder"><header className="builder-header"><div><button className="icon-button" onClick={onBack}><ChevronLeft size={18} /></button><div><h2>{chatbot.name}</h2><span><Status value={chatbot.status} /> · {chatbot.instance.name} · Versão {chatbot.versions[0]?.version}</span></div></div><div>{chatbot.status === 'PUBLISHED' && <Button variant="secondary" onClick={() => changeStatus.mutate('PAUSED')} loading={changeStatus.isPending}><Pause size={15} />Pausar</Button>}{chatbot.status === 'PAUSED' && chatbot.publishedVersion && <Button variant="secondary" onClick={() => changeStatus.mutate('PUBLISHED')} loading={changeStatus.isPending}><Play size={15} />Ativar</Button>}<Button variant="secondary" onClick={() => save.mutate()} loading={save.isPending}><Save size={15} />Salvar</Button><Button onClick={() => publish.mutate()} loading={publish.isPending}><Send size={15} />Publicar</Button></div></header>{error && <div className="builder-error">{error.message}</div>}<div className="builder-body chatbot-builder-body"><aside className="node-palette"><span className="nav-section">Blocos</span>{nodeDefinitions.filter((item) => item.type !== 'trigger').map((item) => <button key={item.type} onClick={() => addNode(item.type)}><span className={item.tone}><item.icon size={15} /></span><div><strong>{item.label}</strong><small>{item.subtitle}</small></div></button>)}<p className="palette-tip">Ciclos são permitidos quando retornam a um bloco de pergunta.</p></aside><div className="flow-canvas"><ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} onNodeClick={(_, node) => setSelectedNodeId(node.id)} onPaneClick={() => setSelectedNodeId(null)} nodeTypes={nodeTypes} fitView colorMode={theme}><Background gap={22} size={1} color={theme === 'dark' ? '#38414a' : '#dfe5ea'} /><Controls /><MiniMap pannable zoomable nodeColor="#2da6dc" maskColor={theme === 'dark' ? 'rgba(25,29,34,.72)' : 'rgba(245,247,249,.75)'} /></ReactFlow></div><NodeInspector node={selectedNode} tags={metadata.tags} onChange={updateSelected} onDelete={deleteSelected} /></div></div>;
}

function NodeInspector({ node, tags, onChange, onDelete }: { node: Node<FlowData> | null; tags: TagOption[]; onChange(changes: Partial<FlowData>): void; onDelete(): void }) {
