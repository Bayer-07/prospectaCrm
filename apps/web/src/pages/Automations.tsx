...
export function AutomationsPage() {
  const client = useQueryClient(); const [selected, setSelected] = useState<WorkflowRecord | null>(null); const [modal, setModal] = useState(false);
  ...
  const workflows = query.data?.data || [];
  return <div className="automations-page"><div className="toolbar"><div className="segmented"><button className="active">Todas</button><button>Publicadas</button><button>Rascunhos</button></div><Button onClick={() => setModal(true)}><Plus size={15} />Nova automação</Button></div>{workflows.length ? <div className="workflow-grid">{workflows.map((workflow) => <button key={workflow.id} onClick={() => setSelected(workflow)}><div className="workflow-icon"><Workflow size={20} /></div><div className="workflow-card-header"><Status value={workflow.status} /><span>v{workflow.versions[0]?.version || 1}</span></div><h3>{workflow.name}</h3><p>{workflow.description || 'Automação de WhatsApp e CRM'}</p><footer><span>{workflow._count?.enrollments || 0} inscrições</span><span>Atualizada {dateTime(workflow.updatedAt)}</span></footer></button>)}</div> : <Empty icon={<Bot />} title="Crie sua primeira automação" description="Monte jornadas com gatilhos, condições, mensagens e ações no CRM." action={<Button onClick={() => setModal(true)}>Nova automação</Button>} />}{modal && <CreateWorkflowModal onClose={() => setModal(false)} onCreated={(workflow) => { setModal(false); setSelected(workflow); }} />}</div>;
}
